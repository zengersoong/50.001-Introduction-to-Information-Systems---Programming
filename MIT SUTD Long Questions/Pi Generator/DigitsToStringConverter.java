...
public class DigitsToStringConverter {
	 ...
	 * 
	 * If digits[i] >= base or digits[i] < 0 for any i, consider the input invalid,
	 * and return null. If alphabet.length != base, consider the input invalid, and
	 * return null.
	 *
	 * @param digits
	...
	public static String convertDigitsToString(int[] digits, int base, char[] alphabet) {
		...
		// if (digits[i]>=base || digits[i] < 0 || alphabet.length != base) return null;
		// else {
		// if (digits[i]>= base || digits[i]<0 || alphabet.length != base) return null;
		// else {
		char[] alphabetNew = new char[digits.length];
		for (int i = 0; i < digits.length; i++) {
			int elementDigit = digits[i];
			char elementAlpha = alphabet[elementDigit];

			alphabetNew[i] = elementAlpha;
		}

		String c = new String(alphabetNew);
		return c;
	}
